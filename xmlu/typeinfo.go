...
func getTypeInfo(typ reflect.Type) (*typeInfo, error) {
	...
	if typ.Kind() == reflect.Struct && typ != nameType {
		...
		for i := 0; i < n; i++ {
			f := typ.Field(i)
			if (!f.IsExported() && !f.Anonymous) || getFinalTag(&f) == "-" {
...
func getFinalTag(f *reflect.StructField) string {
	if f.Tag.Get("xmlu") != "" {
		return f.Tag.Get("xmlu")
	}
	return f.Tag.Get("xml")
}
